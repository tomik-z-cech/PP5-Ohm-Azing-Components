...
class UserProfileForm(forms.ModelForm):
    ...
    marketing_sms = forms.BooleanField(
        label="Can we contact you via SMS ?", required=False
    )
    marketing_phone = forms.BooleanField(
        label="Can we contact you via phone ?", required=False
    )
    marketing_email = forms.BooleanField(
        label="Can we contact you via Email ?", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        """
        Initial method to adjustr form fields
        """
        ...
        self.fields["post_code"].widget.attrs.update(
            {"placeholder": "Post Code (Eir Code)"}
        )
